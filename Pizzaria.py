...
# Cardápio (sempre visível; identifica automaticamente se já há sessão)
    ...
    # Se já há telefone em sessão, confirma identidade
    if st.session_state["telefone"]:
        ...
        if cid:
            ...
            st.info("Telefone em sessão não encontrado no banco. O cardápio permanece visível; cadastre-se para finalizar pedidos.")
    ...
    if not produtos:
        ...
        for idp, nome, preco, imagem in produtos:
            ...
            with col1:
                if imagem:
                    # opcional: pasta static/ — mantenha as imagens lá
                    st.image(f"static/{imagem}", width=120)
            with col2:
                st.write(f"**{nome}** - R$ {preco:.2f}")
                add_key = f"add_{idp}"
                rem_key = f"rem_{idp}"
                if st.button("Adicionar ao carrinho", key=add_key):
                    st.session_state["carrinho"].append(idp)
                if st.button("Remover do carrinho", key=rem_key):
